...
function SponsorSlot({ slot, city, sponsorData, onRefresh, userId, userLat, userLng }) {
  ...
  // Retorna lat/lng resolvidos — corrige o bug de estado assincrono
  const geocodeAddress = async (address) => {
    if (!address || address.trim().length < 5) { alert('Digite um endereco valido!'); return null }
  ...
  const handleSubmit = async () => {
    ...
    // FIX: usa as coordenadas ja no estado ou faz geocode e usa o retorno direto
    let lat = sponsorLat
    let lng = sponsorLng
    if ((!lat || !lng) && form.address) {
      const coords = await geocodeAddress(form.address)
      if (!coords) { alert('Nao foi possivel localizar o endereco.'); return }
      lat = coords.lat
      lng = coords.lng
    }
    ...
    let error
    // FIX: faz UPDATE se o registro ja existe (owner editando OU slot pendente do mesmo usuario)
    if (sponsorData?.id && (isOwner || isPending)) {
      ...
      {showModal && (
        ...
        >
          <div style={{
            ...
            maxWidth: '460px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.4)',
            ...
            {step === 'plan' && (
              <div>
                ...
                <button onClick={() => setStep('pix')} style={{
                  width: '100%', padding: '13px', background: 'linear-gradient(135deg, #667eea, #764ba2)',
                  color: 'white', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '800', cursor: 'pointer'
                }}>
            ...
            {step === 'pix' && (
              <div>
                ...
                <div style={{ background: '#f0fdf4', border: '2px solid #16a34a', borderRadius: '14px', padding: '16px', marginBottom: '14px', textAlign: 'center' }}>
                  ...
                  <button onClick={() => navigator.clipboard && navigator.clipboard.writeText(PIX_KEY)} style={{ padding: '7px 16px', background: '#16a34a', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '13px', fontWeight: '700', marginTop: '4px' }}>
                    Copiar chave Pix
                  </button>
                </div>
                ...
                <div style={{ display: 'flex', gap: '12px', marginTop: '4px' }}>
                  ...
                  <button onClick={() => setStep('form')} style={{ flex: 2, padding: '14px', background: 'linear-gradient(135deg, #16a34a, #15803d)', color: 'white', border: 'none', borderRadius: '12px', cursor: 'pointer', fontWeight: '800', fontSize: '16px', boxShadow: '0 4px 14px rgba(22,163,74,0.35)' }}>✅ Ja paguei</button>
                </div>
              </div>
            )}
              ...
              <div>
                ...
                <div style={{ marginBottom: '14px' }}>
                  ...
                  <div style={{ display: 'flex', gap: '6px' }}>
                    ...
                    />
                    <button onClick={() => geocodeAddress(form.address)} style={{ padding: '9px 12px', background: '#667eea', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontSize: '12px', fontWeight: '700', whiteSpace: 'nowrap' }}>
                      Localizar
                    </button>
                  </div>
            ...
            {step === 'success' && (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <button onClick={() => setShowModal(false)} style={{ padding: '12px 28px', background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '800', cursor: 'pointer' }}>
                  Fechar
                </button>
              </div>
            )}
