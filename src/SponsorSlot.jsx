...
// userLat/userLng: posicao atual do visitante
function SponsorSlot({ slot, city, sponsorData, onRefresh, userId, userLat, userLng }) {
  ...
  // Verifica se o visitante esta dentro do raio de 2km do patrocinador
  const dentroDoRaio = () => {
            if (!isActive) return false
            // Se patrocinador nao tem coordenadas, mostra para todos (compatibilidade)
            if (!sponsorData.lat || !sponsorData.lng) return true
            // Se nao temos localizacao do usuario, mostra para todos
            if (!userLat || !userLng) return true
            const dist = calcDistKm(userLat, userLng, sponsorData.lat, sponsorData.lng)
            return dist <= RAIO_KM
  }

  const visivelParaUsuario = isOwner || dentroDoRaio() || isPending || !isActive

  const handleSponsorClick = () => {
            if (!visivelParaUsuario && isActive) {
                        // Fora do raio: slot livre para novo patrocinador
              setStep('plan')
                        setShowModal(true)
                        return
            }
            ...
            if (isOwner) {
                        setForm({
                                      ...
                                      offers: sponsorData.offers && sponsorData.offers.length === 5 ? sponsorData.offers : ['', '', '', '', ''],
  ...
  // Geocodifica endereco para obter lat/lng usando API publica
  const geocodeAddress = async (address) => {
            if (!address || address.trim().length < 5) {
                        alert('Digite um endereço válido para localização!')
                        return false
            }
            setGeoStatus('Buscando localização...')
            try {
                        const query = encodeURIComponent(address + ', ' + city + ', Brasil')
                        const res = await fetch('https://nominatim.openstreetmap.org/search?q=' + query + '&format=json&limit=1')
                        const data = await res.json()
                        if (data && data.length > 0) {
                                      setSponsorLat(parseFloat(data[0].lat))
                                      setSponsorLng(parseFloat(data[0].lon))
                                      setGeoStatus('Localização encontrada! ' + parseFloat(data[0].lat).toFixed(4) + ', ' + parseFloat(data[0].lon).toFixed(4))
                                      return true
                        } else {
                                      setGeoStatus('Endereço não encontrado. Tente ser mais específico.')
                                      return false
                        }
            } catch {
                        setGeoStatus('Erro ao buscar localização.')
                        return false
            }
  }
  ...
  const handleSubmit = async () => {
            ...
            if ((!lat || !lng) && form.address) {
                        ...
                        if (!ok) {
                                      alert('Não foi possível localizar o endereço. Verifique e tente novamente.')
                                      return
                        }
                        lat = sponsorLat
                        lng = sponsorLng
  ...
  // Conteudo do slot conforme estado e visibilidade
  const renderSlotContent = () => {
            // Slot ativo mas usuario esta fora do raio: slot livre para novo patrocinador
                                            ...
                                            {sponsorData.offers && sponsorData.offers.length > 0 && (
                                                        <div>
                                                              {sponsorData.offers.slice(0, 2).map((o, i) => (
                                                                              <div key={i} style={{ fontSize: '10px', color: '#dc2626', fontWeight: '700', lineHeight: '1.3' }}>🔥 {o}</div>div>
                                                                            ))}
                                                        </div>div>
                                                )}
          ...
          const successMsg = isOwner
                    ? 'Seu espaco de patrocinador foi atualizado! Ele aparece para visitantes num raio de ' + RAIO_KM + 'km do seu estabelecimento.'
                    : 'Recebemos seus dados. Apos confirmarmos o pagamento Pix, seu espaco sera ativado em ate 24h e ficara visivel para usuarios num raio de ' + RAIO_KM + 'km do seu estabelecimento.'
                
                  const slotBorderColor = isActive
                            ? (isOwner || dentroDoRaio() ? '#fbbf24' : '#94a3b8')
                            : isPending ? '#d97706' : '#94a3b8'
                          ...
                          const slotBg = isActive
                                  ...
                                  const slotBorder = isActive
                                            ? (isOwner || dentroDoRaio() ? '2px solid ' + slotBorderColor : '2px dashed #94a3b8')
                                          ...
                                          return (
                                                    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
                                                          <div
                                                                        ...
                                                                        title={
                                                                                        isActive && !isOwner && !dentroDoRaio()
                                                                                          ? 'Espaço disponível nesta região!'
                                                                                          : isActive ? sponsorData.sponsor_name
                                                                                          : isPending ? 'Aguardando pagamento'
                                                                                          : 'Anuncie aqui num raio de ' + RAIO_KM + 'km!'
                                                                        }
                                                                      >
                                                                {renderSlotContent()}
                                                          </div>div>
                                                          ...
                                                          {showModal && (
                                                                  <div
                                                                            ...
                                                                            <div style={{ background: 'white', borderRadius: '20px', padding: '28px', maxWidth: '460px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', maxHeight: '92vh', overflowY: 'auto', position: 'relative' }}>
                                                                                        <button onClick={() => setShowModal(false)} style={{ position: 'absolute', top: '14px', right: '18px', background: 'none', border: 'none', fontSize: '22px', cursor: 'pointer', color: '#888' }}>✕</button>button>
                                                                                  ...
                                                                                  {step === 'plan' && (
                                                                                                    <div>
                                                                                                                    ...
                                                                                                                    <p style={{ color: '#666', fontSize: '13px', marginBottom: '8px' }}>
                                                                                                                                      Seu anúncio aparece para visitantes num raio de <strong>{RAIO_KM}km</strong>strong> do seu estabelecimento em <strong>{city}</strong>strong>!
                                                                                                                          </p>p>
                                                                                                                    <div style={{ background: '#eef2ff', border: '2px solid #667eea', borderRadius: '10px', padding: '10px', marginBottom: '16px', fontSize: '12px', color: '#374151' }}>
                                                                                                                                      📍 <strong>Visibilidade por localização:</strong>strong> Somente usuários que estiverem a até {RAIO_KM}km do seu estabelecimento verão seu anúncio. Fora desse raio, o espaço fica disponível para outro patrocinador da região.
                                                                                                                          </div>div>
                                                                                                          ...
                                                                                                          { type: 'yearly', label: 'Anual', price: 'R$ 400,00', sub: 'por ano · economize R$ 200!' }
                                                                                                                                                      ...
                                                                                                                                                      <div key={p.type} onClick={() => setPlanType(p.type)}
                                                                                                                                                                                  style={{ flex: 1, padding: '14px 10px', border: planType === p.type ? '3px solid #667eea' : '2px solid #e2e8f0', borderRadius: '14px', cursor: 'pointer', textAlign: 'center', background: planType === p.type ? '#eef2ff' : 'white' }}>
                                                                                                                    ...
                                                                                                                    <div style={{ background: '#f8fafc', borderRadius: '10px', padding: '12px', marginBottom: '16px', fontSize: '12px', color: '#374151' }}>
                                                                                                                                      <div style={{ fontWeight: '700', marginBottom: '5px' }}>✅ O que você recebe:</div>div>
                                                                                                                                      <div>• Logo/foto do estabelecimento no espaço</div>div>
                                                                                                                                      <div>• Até 5 super ofertas visíveis no raio de {RAIO_KM}km</div>div>
                                                                                                                                      <div>• Link clicável para seu site ou rede social</div>div>
                                                                                                                                      <div>• Visibilidade exclusiva para quem está perto de você</div>div>
                                                                                                                                      <div>• Edite seu espaço quando quiser</div>div>
                                                                                                                          </div>div>
                                                                                                                    <button onClick={() => setStep('pix')}
                                                                                                                                            style={{ width: '100%', padding: '13px', background: 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', border: 'none', borderRadius: '12px', fontSize: '15px', fontWeight: '800', cursor: 'pointer' }}>
                                                                                  ...
                                                                                  {step === 'pix' && (
                                                                                                    <div>
                                                                                                                    ...
                                                                                                                    <p style={{ color: '#666', fontSize: '13px', marginBottom: '16px' }}>Plano {planLabel} — <strong>R$ {planPrice},00</strong>strong></p>p>
                                                                                                                    <div style={{ background: '#f0fdf4', border: '2px solid #16a34a', borderRadius: '14px', padding: '16px', marginBottom: '14px', textAlign: 'center' }}>
                                                                                                                                      ...
                                                                                                                                      <div style={{ fontSize: '12px', color: '#666' }}>Beneficiário: <strong>{PIX_NAME}</strong>strong></div>div>
                                                                                                                    ...
                                                                                                                    <div style={{ background: '#fffbeb', border: '1px solid #fbbf24', borderRadius: '10px', padding: '10px', fontSize: '12px', color: '#92400e', marginBottom: '14px' }}>
                                                                                                                                      ⚠️ Após o pagamento, clique em &quot;Já paguei&quot; e preencha seus dados. Ativamos em até 24h.
                                                                                                                          </div>div>
                                                                                                                    <div style={{ display: 'flex', gap: '10px' }}>
                                                                                                                                      <button onClick={() => setStep('plan')} style={{ flex: 1, padding: '11px', background: '#f1f5f9', color: '#374151', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600' }}>← Voltar</button>button>
                                                                                                                                      <button onClick={() => setStep('form')} style={{ flex: 2, padding: '11px', background: 'linear-gradient(135deg, #16a34a, #15803d)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '800', fontSize: '14px' }}>✅ Já paguei →</button>button>
                                                                                                                          </div>div>
                                                                                                          </div>div>
                                                                                        )}
                                                                                  ...
                                                                                  {step === 'form' && (
                                                                                                    <div>
                                                                                                                    <h2 style={{ margin: '0 0 4px', fontSize: '20px', color: '#1a202c' }}>{isOwner ? '✏️ Editar espaço' : '📝 Seus dados'}</h2>h2>
                                                                                                                    <p style={{ color: '#666', fontSize: '12px', marginBottom: '14px' }}>
                                                                                                                          {isOwner ? 'Atualize as informações do seu espaço.' : 'Preencha para ativarmos seu espaço no raio de ' + RAIO_KM + 'km.'}
                                                                                                                          </p>p>
                                                                                                    
                                                                                                          {/* LOGO */}
                                                                                                                    <div style={{ marginBottom: '14px', textAlign: 'center' }}>
                                                                                                                                      <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#374151', marginBottom: '6px' }}>
                                                                                                                                                          🖼️ Logo / Foto do estabelecimento
                                                                                                                                            </label>label>
                                                                                                                          {logoPreview && (
                                                                                                                              <img src={logoPreview} alt="preview" style={{ width: '110px', height: '70px', objectFit: 'contain', borderRadius: '8px', border: '2px solid #e2e8f0', marginBottom: '6px', display: 'block', margin: '0 auto 6px' }} />
                                                                                                                            )}
                                                                                                                                      <input ref={fileInputRef} type="file" accept="image/*" onChange={handleLogoUpload} style={{ display: 'none' }} />
                                                                                                                                      <button onClick={() => fileInputRef.current && fileInputRef.current.click()}
                                                                                                                                                                disabled={uploadingLogo}
                                                                                                                                                                style={{ padding: '8px 18px', background: uploadingLogo ? '#aaa' : '#667eea', color: 'white', border: 'none', borderRadius: '8px', cursor: uploadingLogo ? 'not-allowed' : 'pointer', fontSize: '12px', fontWeight: '700' }}>
                                                                                                          ...
                                                                                                          {/* CAMPOS BASICOS */}
                                                                                                          {[
                                                                                                          { key: 'name', label: 'Nome do negócio *', placeholder: 'Ex: Pizzaria do João' },
                                                                                                          { key: 'email', label: 'E-mail para contato *', placeholder: 'seu@email.com' },
                                                                                                          { key: 'phone', label: 'WhatsApp / Telefone', placeholder: '(42) 99999-9999' },
                                                                                                          { key: 'link_url', label: '🔗 URL (torna seu espaço clicável)', placeholder: 'https://instagram.com/seunegocio' }
                                                                                                                          ].map(f => (
                                                                                                                                                  <div key={f.key} style={{ marginBottom: '10px' }}>
                                                                                                                                                                      <label style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#374151', marginBottom: '3px' }}>{f.label}</label>label>
                                                                                                                                                                      <input
                                                                                                                                                                                                  value={form[f.key]}
                                                                                                          ...
                                                                                                          {/* ENDERECO + GEO */}
                                                                                                                    <div style={{ marginBottom: '14px' }}>
                                                                                                                                      <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#1e3a8a', marginBottom: '3px' }}>
                                                                                                                                                          📍 Endereço do estabelecimento * (para definir o raio de {RAIO_KM}km)
                                                                                                                                            </label>label>
                                                                                                                                      <div style={{ display: 'flex', gap: '6px' }}>
                                                                                                                                                          <input
                                                                                                                                                                                      value={form.address}
                                                                                                                                                                                      onChange={e => { setForm(prev => ({ ...prev, address: e.target.value })); setGeoStatus(''); setSponsorLat(null); setSponsorLng(null) }}
                                                                                                                          ...
                                                                                                                          {sponsorLat && sponsorLng && (
                                                                                                                              <div style={{ fontSize: '11px', marginTop: '4px', color: '#15803d', background: '#f0fdf4', padding: '6px 10px', borderRadius: '6px' }}>
                                                                                                                                                    ✅ Coordenadas registradas — seu anúncio aparecerá para usuários num raio de {RAIO_KM}km
                                                                                                                                    </div>div>
                                                                                                                                      )}
                                                                                                                          {!sponsorLat && !geoStatus && (
                                                                                                                              <div style={{ fontSize: '11px', marginTop: '4px', color: '#d97706' }}>
                                                                                                                                                    ⚠️ Clique em Localizar após digitar o endereço
                                                                                                                                    </div>div>
                                                                                                                                      )}
                                                                                                                          </div>div>
                                                                                                          ...
                                                                                                          {/* 5 SUPER OFERTAS */}
                                                                                                                    <div style={{ marginBottom: '14px' }}>
                                                                                                                                      <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#dc2626', marginBottom: '6px' }}>
                                                                                                                                                          🔥 Até 5 Super Ofertas (visíveis no raio de {RAIO_KM}km)
                                                                                                                                            </label>label>
                                                                                                          ...
                                                                                                          {isPending && (
                                                                                                                            <div style={{ background: '#fef3c7', border: '2px solid #fbbf24', borderRadius: '10px', padding: '10px', marginBottom: '14px', textAlign: 'center' }}>
                                                                                                                                                ...
                                                                                                                                                <div style={{ fontSize: '11px', color: '#92400e', marginTop: '3px' }}>Após confirmação do pagamento seu espaço será ativado no raio de {RAIO_KM}km.</div>div>
                                                                                                                                  </div>div>
                                                                                                                    )}
                                                                                                    
                                                                                                                    <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
                                                                                                                          {!isOwner && (
                                                                                                                              <button onClick={() => setStep('pix')} style={{ flex: 1, padding: '11px', background: '#f1f5f9', color: '#374151', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600' }}>← Voltar</button>button>
                                                                                                                                      )}
                                                                                                                                      <button onClick={handleSubmit} disabled={saving}
                                                                                                                                                                style={{ flex: 2, padding: '11px', background: saving ? '#aaa' : 'linear-gradient(135deg, #667eea, #764ba2)', color: 'white', border: 'none', borderRadius: '10px', cursor: saving ? 'not-allowed' : 'pointer', fontWeight: '800', fontSize: '14px' }}>
                                                                                  ...
                                                                                  {step === 'success' && (
                                                                                                    <div style={{ textAlign: 'center', padding: '20px 0' }}>
                                                                                                                    ...
                                                                                                                    <h2 style={{ color: '#15803d', marginBottom: '8px' }}>{isOwner ? 'Alterações salvas!' : 'Cadastro enviado!'}</h2>h2>
