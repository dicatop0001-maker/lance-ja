...
function SponsorSlot({ slot, city, sponsorData, onRefresh, userId }) {
    ...
    const [step, setStep] = useState('plan') // plan | pix | form | preview | success
                            ...
                            {sponsorData.offers && sponsorData.offers.length > 0 && (
                                        <div style={{ fontSize: '10px', color: '#374151', lineHeight: '1.3' }}>
                                          {sponsorData.offers.slice(0, 2).map((o, i) => (
                                                          <div key={i} style={{ color: '#dc2626', fontWeight: '700' }}>🔥 {o}</div>div>
                                                        ))}
                                        </div>div>
                                      )}
          ...
          {showModal && (
                  <div
                            ...
                            <div style={{ background: 'white', borderRadius: '20px', padding: '28px', maxWidth: '460px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', maxHeight: '92vh', overflowY: 'auto', position: 'relative' }}>
                              ...
                              {step === 'plan' && (
                                            <>
                                                            ...
                                                            <p style={{ color: '#666', fontSize: '13px', marginBottom: '20px' }}>
                                                                              Seu negócio aparece na home do <strong>Zap Bairro</strong>strong> em <strong>{city}</strong>strong> para todos os usuários!
                                                            </p>p>
                                              ...
                                              { type: 'yearly', label: 'Anual', price: 'R$ 400,00', sub: 'por ano · economize R$ 200!' }
                                                            ...
                                                            <div style={{ background: '#f8fafc', borderRadius: '12px', padding: '14px', marginBottom: '18px', fontSize: '13px', color: '#374151' }}>
                                                                              ...
                                                                              <div>• Espaço exclusivo com sua logo na home do Zap Bairro</div>div>
                                                                              <div>• Até 5 super ofertas visíveis para todos os visitantes</div>div>
                                                                              <div>• Link clicável direto para seu site ou rede social</div>div>
                              ...
                              {step === 'pix' && (
                                            <>
                                                            ...
                                                            <div style={{ display: 'flex', gap: '10px' }}>
                                                                              ...
                                                                              <button onClick={() => setStep('form')} style={{ flex: 2, padding: '12px', background: 'linear-gradient(135deg, #16a34a, #15803d)', color: 'white', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '800', fontSize: '15px' }}>✅ Já paguei →</button>button>
                                                            </div>div>
                                            </>>
                                          )}
                              ...
                              {step === 'form' && (
                                            <>
                                                            <h2 style={{ margin: '0 0 6px', fontSize: '20px', color: '#1a202c' }}>📝 {isOwner ? 'Editar espaço' : 'Seus dados'}</h2>h2>
                                                            <p style={{ color: '#666', fontSize: '13px', marginBottom: '16px' }}>
                                                              {isOwner ? 'Atualize as informações do seu espaço de patrocinador.' : 'Preencha para ativarmos seu espaço.'}
                                                              {isPending && !isOwner && <span style={{ color: '#d97706', fontWeight: '700' }}><br/>⏳ Aguardando confirmação do pagamento.</span>span>}
                                                            </p>p>
                                              ...
                                              { key: 'link_url', label: '🔗 URL do seu site/rede social (opcional — torna seu espaço clicável)', placeholder: 'https://instagram.com/seunegocio' }
                                                              ].map(f => (
                                              ...
                                              {/* 5 SUPER OFERTAS */}
                                                            <div style={{ marginBottom: '16px' }}>
                                                                              <label style={{ display: 'block', fontSize: '13px', fontWeight: '700', color: '#dc2626', marginBottom: '8px' }}>
                                                                                                  🔥 Até 5 Super Ofertas (aparecem no seu espaço)
                                                                              </label>label>
                                              ...
                                              {isPending && !isOwner && (
                                                                <div style={{ background: '#fef3c7', border: '2px solid #fbbf24', borderRadius: '10px', padding: '12px', marginBottom: '16px', textAlign: 'center' }}>
                                                                                    ...
                                                                                    <div style={{ fontSize: '12px', color: '#92400e', marginTop: '4px' }}>Após confirmação do pagamento seu espaço será ativado.</div>div>
                                                                </div>div>
                                                            )}
                                            
                                                            <div style={{ display: 'flex', gap: '10px', marginTop: '8px' }}>
                                                              {!isOwner && <button onClick={() => setStep('pix')} style={{ flex: 1, padding: '12px', background: '#f1f5f9', color: '#374151', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '600' }}>← Voltar</button>button>}
                                                            ...
                                                            <p style={{ color: '#374151', fontSize: '14px', marginBottom: '20px' }}>
                                                              {isOwner
                                                                                    ...
                                                                                    : <>Recebemos seus dados. Após confirmarmos o pagamento Pix, seu espaço será ativado em até 24h.<br /><br />
                                                                                                          Você receberá um aviso no e-mail <strong>{form.email}</strong>strong> antes do vencimento para renovar.</>>
                                                              }
                                                            </p>p>
