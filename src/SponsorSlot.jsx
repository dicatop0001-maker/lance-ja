...
function SponsorSlot({ slot, city, sponsorData, onRefresh, userId, userLat, userLng }) {
  ...
  const [form, setForm] = useState({
    name: '', email: '', phone: '', link_url: '',
    offers: ['', '', '', '', ''], logo_url: '', address: ''
  })
  ...
  // Retorna lat/lng resolvidos — corrige o bug de estado assíncrono
  const geocodeAddress = async (address) => {
    if (!address || address.trim().length < 5) { alert('Digite um endereço válido!'); return null }
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        setSponsorLat(lat)
        setSponsorLng(lng)
        setGeoStatus('Localização encontrada! ' + lat.toFixed(4) + ', ' + lng.toFixed(4))
        return { lat, lng }
      }
  ...
  const handleSubmit = async () => {
    ...
    // FIX: usa as coordenadas já no estado ou faz geocode e usa o retorno direto
    let lat = sponsorLat
    let lng = sponsorLng

    if ((!lat || !lng) && form.address) {
      const coords = await geocodeAddress(form.address)
      if (!coords) { alert('Não foi possível localizar o endereço.'); return }
      lat = coords.lat
      lng = coords.lng
    }
    ...
    if (isPending) {
      return (
        <div style={{ textAlign: 'center', padding: '4px' }}>
          ...
          {isOwner && <div style={{ fontSize: '9px', color: '#666', marginTop: '3px' }}>Em análise</div>}
        </div>
      )
    }
    return (
  ...
  const successMsg = isOwner
    ? 'Seu espaço foi atualizado com sucesso!'
    : 'Recebemos seus dados. Após confirmarmos o pagamento, seu espaço será ativado.'
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column' }}>
      <div
        ...
        style={{
          flex: 1, minHeight: '120px', border: slotBorder, borderRadius: '10px',
          background: slotBg, cursor: 'pointer', display: 'flex', flexDirection: 'column',
          alignItems: 'center', justifyContent: 'center', padding: '8px',
          transition: 'box-shadow 0.2s, transform 0.2s', boxSizing: 'border-box', overflow: 'hidden'
        }}
      ...
      {showModal && (
        ...
        >
          <div style={{ background: 'white', borderRadius: '20px', padding: '28px', maxWidth: '460px', width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.4)', maxHeight: '92vh', overflowY: 'auto', position: 'relative' }}>
            <button onClick={() => setShowModal(false)} style={{ position: 'absolute', top: '14px', right: '18px', background: 'none', border: 'none', fontSize: '22px', cursor: 'pointer', color: '#888' }}>✕</button>
            ...
            {step === 'plan' && (
              <div>
                ...
                <div style={{ background: '#fff7ed', border: '2px solid #f97316', borderRadius: '10px', padding: '10px', marginBottom: '16px', fontSize: '12px', color: '#7c2d12' }}>
                  Benefícios do patrocinador: Um anúncio criado pelo patrocinador ficará sempre entre os 6 primeiros no feed de anúncios. Cada patrocinador terá o direito de fazer um anúncio para ficar no topo do feed.
                </div>
                <div style={{ background: '#f8fafc', borderRadius: '10px', padding: '12px', marginBottom: '16px', fontSize: '12px', color: '#374151' }}>
                  <div style={{ fontWeight: '700', marginBottom: '5px' }}>O que você recebe:</div>
                  <div>✅ Logo/foto do estabelecimento no espaço</div>
                  <div>✅ Até 5 super ofertas exclusivas</div>
                  <div>✅ Link clicável para seu site ou rede social</div>
                  <div>✅ Visibilidade exclusiva para clientes próximos</div>
                  <div>✅ Edite seu espaço quando quiser</div>
                </div>
            ...
            {step === 'pix' && (
              <div>
                ...
                <div style={{ background: '#fffbeb', border: '1px solid #fbbf24', borderRadius: '10px', padding: '10px', fontSize: '12px', color: '#92400e', marginBottom: '14px' }}>
                  Após o pagamento, clique em "Já paguei" e preencha seus dados. Ativamos em até 24h.
                </div>
            ...
            {step === 'form' && (
              <div>
                <h2 style={{ margin: '0 0 4px', fontSize: '20px', color: '#1a202c' }}>{isOwner ? 'Editar espaço' : 'Seus dados'}</h2>
                <p style={{ color: '#666', fontSize: '12px', marginBottom: '14px' }}>{isOwner ? 'Atualize as informações do seu espaço.' : 'Preencha seus dados para ativarmos seu espaço.'}</p>
                ...
                <div style={{ marginBottom: '14px', textAlign: 'center' }}>
                  ...
                  <button onClick={() => fileInputRef.current && fileInputRef.current.click()} disabled={uploadingLogo}
                ...
                {[
                  ...
                  { key: 'link_url', label: 'URL do site/rede social (torna clicável)', placeholder: 'https://instagram.com/seunegocio' }
                ].map(f => (
                ...
                <div style={{ marginBottom: '14px' }}>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#1e3a8a', marginBottom: '3px' }}>Endereço do estabelecimento *</label>
                  <div style={{ display: 'flex', gap: '6px' }}>
                    ...
                    <button onClick={() => geocodeAddress(form.address)}
                  ...
                  {sponsorLat && sponsorLng && <div style={{ fontSize: '11px', marginTop: '4px', color: '#15803d', background: '#f0fdf4', padding: '6px 10px', borderRadius: '6px' }}>Localização registrada com sucesso!</div>}
                </div>

                <div style={{ marginBottom: '14px' }}>
                  <label style={{ display: 'block', fontSize: '12px', fontWeight: '700', color: '#dc2626', marginBottom: '6px' }}>Até 5 Super Ofertas</label>
                  {form.offers.map((offer, i) => (
                    <div key={i} style={{ marginBottom: '7px' }}>
                      <input value={offer}
                        onChange={e => { const newOffers = [...form.offers]; newOffers[i] = e.target.value; setForm(prev => ({ ...prev, offers: newOffers })) }}
                ...
                {isPending && (
                  <div style={{ background: '#fef3c7', border: '2px solid #fbbf24', borderRadius: '10px', padding: '10px', marginBottom: '14px', textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: '11px', color: '#92400e', marginTop: '3px' }}>Após confirmação do pagamento seu espaço será ativado.</div>
                  </div>
                )}
                ...
                <div style={{ display: 'flex', gap: '12px', marginTop: '20px', paddingTop: '16px', borderTop: '2px solid #e2e8f0' }}>
                  ...
                  <button onClick={handleSubmit} disabled={saving}
                    style={{ flex: 2, padding: '15px', background: saving ? '#94a3b8' : 'linear-gradient(135deg, #f97316, #ea580c)', color: 'white', border: 'none', borderRadius: '12px', cursor: saving ? 'not-allowed' : 'pointer', fontWeight: '800', fontSize: '17px', boxShadow: saving ? 'none' : '0 4px 16px rgba(249,115,22,0.45)', letterSpacing: '0.3px' }}>
            ...
            {step === 'success' && (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <h2 style={{ color: '#15803d', marginBottom: '8px' }}>{isOwner ? 'Alterações salvas!' : 'Cadastro enviado!'}</h2>
