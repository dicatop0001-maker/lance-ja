...
function EditarAnuncio() {
  ...
  const loadAuction = async () => {
    ...
    if (error || !data) { alert('Anúncio não encontrado'); navigate('/home'); return }
    if (data.seller_id !== user.id) { alert('Você não tem permissão para editar este anúncio'); navigate('/home'); return }
  ...
  const handlePhotoUpload = async (e) => {
    ...
    for (const file of files) {
      try {
        ...
        if (!res.ok || data.error) { alert('Erro ao enviar foto: ' + (data.error || res.statusText)) }
  ...
  const handleSave = async (e) => {
    ...
    else { alert('Anúncio atualizado com sucesso!'); navigate('/home') }
  }
  ...
  const handleDelete = async () => {
    ...
    else { alert('Anúncio excluído!'); navigate('/home') }
  }
  ...
  if (loading) return <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f97316, #ea580c)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontSize: '20px' }}>Carregando...</div>
    ...
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #f97316 0%, #ea580c 100%)', padding: '20px' }}>
      <div style={{ maxWidth: '600px', margin: '0 auto', background: 'white', borderRadius: '20px', padding: '28px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '24px' }}>
          ...
          <h1 style={{ margin: 0, fontSize: '22px', fontWeight: '800', color: '#1a202c' }}>✏️ Editar Anúncio</h1>
        </div>
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <div>
            ...
            <div style={{ position: 'relative' }}>
              ...
              {cityState && <div style={{ fontSize: '12px', color: '#f97316', fontWeight: '600', marginTop: '3px' }}>Estado: {cityState}</div>}
              {showCityDropdown && (
                <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'white', border: '2px solid #e2e8f0', borderRadius: '10px', zIndex: 999, boxShadow: '0 8px 24px rgba(0,0,0,0.15)', maxHeight: '180px', overflowY: 'auto' }}>
                  {filteredCities.map((c, i) => (
                    <div key={i} onMouseDown={() => selectCity(c)} style={{ padding: '10px 14px', cursor: 'pointer', borderBottom: '1px solid #f0f0f0', fontSize: '14px' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '10px' }}>
              {photos.map((p, i) => (
                <div key={i} style={{ position: 'relative' }}>
                  <img src={p} alt="foto" style={{ width: '72px', height: '72px', objectFit: 'cover', borderRadius: '8px', border: '2px solid #f97316' }} />
              ...
              {photos.length < 5 && (
                <label style={{ width: '72px', height: '72px', border: '2px dashed #f97316', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '24px', color: '#f97316' }}>
                  +<input type="file" accept="image/*" multiple onChange={handlePhotoUpload} style={{ display: 'none' }} />
                </label>
              )}
            </div>
            {uploading && <p style={{ color: '#f97316', fontSize: '13px' }}>⏳ Enviando fotos...</p>}
          </div>

          <button type="submit" disabled={saving || uploading} style={{ padding: '16px', background: saving ? '#aaa' : 'linear-gradient(135deg, #f97316, #ea580c)', color: 'white', border: 'none', borderRadius: '12px', fontSize: '17px', fontWeight: '800', cursor: saving ? 'not-allowed' : 'pointer' }}>
