    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #FF6B35 0%, #004E89 100%)', position: 'relative', overflow: 'hidden' }}>
      ...
      <div className="animate-scale-in" style={{ background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(20px)', padding: '48px', borderRadius: '24px', boxShadow: '0 20px 60px rgba(0,0,0,0.2)', width: '100%', maxWidth: '480px', position: 'relative', zIndex: 1 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: '40px' }}>
          ...
          <div style={{ background: 'white', borderRadius: '0 0 10px 10px', border: '2px solid #e2e8f0', borderTop: 'none', overflow: 'hidden', height: '130px', position: 'relative', lineHeight: 0 }}>
            <img src="/logo-conecty.png" alt="Conecty" style={{ display: 'block', width: '100%', position: 'absolute', left: 0, top: '50%', transform: 'translateY(-50%)' }} />
          </div>
        </div>
        ...
        <form onSubmit={handleAuth} style={{ marginBottom: '24px' }}>
          ...
          <button type="submit" disabled={loading}
            style={{ width: '100%', padding: '18px', background: loading ? 'var(--gray-400)' : 'linear-gradient(135deg, var(--primary) 0%, var(--primary-dark) 100%)', color: 'white', border: 'none', borderRadius: '12px', fontSize: '18px', fontWeight: '700', fontFamily: 'var(--font-display)', cursor: loading ? 'not-allowed' : 'pointer', transition: 'all 0.3s', boxShadow: loading ? 'none' : '0 4px 16px rgba(255,107,53,0.3)' }}
        ...
        <button onClick={() => setIsLogin(!isLogin)}
          style={{ width: '100%', padding: '16px', background: 'white', color: 'var(--primary)', border: '2px solid var(--primary)', borderRadius: '12px', fontSize: '16px', fontWeight: '600', fontFamily: 'var(--font-display)', cursor: 'pointer', transition: 'all 0.2s' }}
