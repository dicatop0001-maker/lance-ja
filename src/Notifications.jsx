...
function Notifications({ user, bottomBar }) {
  ...
  return (
    <div style={{ position: 'relative', display: 'flex', alignItems: 'center' }}>
      ...
      >
        <span translate="no">🔔 Notificações</span>
        {unreadCount > 0 && (
          <span style={{
            position: 'absolute',
            top: '-8px',
      ...
      {showDropdown && (
        <div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
            <h3 translate="no" style={{ margin: 0, fontSize: '18px', color: '#333' }}>Notificações</h3>
            {unreadCount > 0 && (
              <button
          ...
          {notifications.length === 0 ? (
            <p translate="no" style={{ color: '#999', textAlign: 'center', padding: '20px' }}>Nenhuma notificação</p>
          ) : (
