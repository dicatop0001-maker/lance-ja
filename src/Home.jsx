...
function getTimeLeft(endsAt) {
  ...
  if (diffMin < 60) {
    return { label: diffMin + ' min restantes', urgent: true }
  } else if (diffH < 24) {
    return { label: diffH + (diffH === 1 ? ' hora restante' : ' horas restantes'), urgent: true }
  } else if (diffD === 1) {
    return { label: '1 dia restante', urgent: false }
  } else {
...
function Home() {
  ...
  useEffect(() => {
    ...
    const sameNeighborhood = filtered.filter(a =>
      userNeighborhood && a.neighborhood &&
      a.neighborhood.toLowerCase().trim() === userNeighborhood.toLowerCase().trim()
    )
    const otherNeighborhoods = filtered.filter(a =>
      !(userNeighborhood && a.neighborhood &&
        a.neighborhood.toLowerCase().trim() === userNeighborhood.toLowerCase().trim())
    )
    setActiveAuctions([...sameNeighborhood, ...otherNeighborhoods])
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)' }}>
      ...
      <div style={{ padding: '30px 20px 10px', textAlign: 'center' }}>
        <h3 style={{ color: 'white', fontSize: 'clamp(32px, 7vw, 60px)', fontWeight: 'bold', margin: '0 0 6px 0' }}>{userCity} - {userState}</h3>
        <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: 'clamp(14px, 2.5vw, 20px)', margin: '0 0 10px 0' }}>veículos, objetos, móveis e imóveis</p>
        <div style={{ margin: '0 auto 14px', maxWidth: '360px' }}>
          <input type="text" value={userNeighborhood} onChange={e => setUserNeighborhood(e.target.value)} placeholder="📍 Seu bairro (para priorizar perto de você)" style={{ width: '100%', padding: '11px 18px', borderRadius: '30px', border: '2px solid rgba(255,255,255,0.6)', background: 'rgba(255,255,255,0.18)', color: 'white', fontSize: '14px', fontWeight: '600', outline: 'none', boxSizing: 'border-box', textAlign: 'center' }} />
        </div>
        <button onClick={() => setShowCitySearch(!showCitySearch)} style={{ padding: '14px 28px', background: 'rgba(255,255,255,0.3)', color: 'white', border: '2px solid white', borderRadius: '15px', fontSize: 'clamp(14px, 2vw, 18px)', cursor: 'pointer', fontWeight: 'bold', marginBottom: '16px' }}>Buscar em outra cidade</button>
        {showCitySearch && (
      ...
      <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '0 16px 40px' }}>
        ...
        <h2 style={{ color: 'white', fontSize: 'clamp(22px, 4vw, 32px)', fontWeight: 'bold', marginBottom: '16px' }}>Leilões e itens Ativos</h2>
        <div style={{ background: 'rgba(255,255,255,0.15)', borderRadius: '16px', padding: '14px 16px', marginBottom: '24px', display: 'flex', flexDirection: 'column', gap: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', background: 'white', borderRadius: '30px', padding: '8px 16px', gap: '10px' }}>
            ...
            {searchTerm && (<button onClick={() => setSearchTerm('')} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: '18px', color: '#999', padding: '0' }}>X</button>)}
          </div>
          <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', justifyContent: 'center' }}>
            {[{val:'',label:'Todos'},{val:'veiculos',label:'Veículos'},{val:'eletronicos',label:'Eletrônicos, Máquinas, Celulares'},{val:'objetos',label:'Objetos'},{val:'moveis',label:'Móveis'},{val:'imoveis',label:'Imóveis'},{val:'outros',label:'Outros'},{val:'servicos',label:'Serviços (lance menor vence)'}].map(cat => (
        ...
        {loading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '20px' }}>
            {displayedAuctions.map(auction => {
              ...
              const isSameNeighborhood = userNeighborhood && auction.neighborhood &&
              ...
              return (
                <div key={auction.id} onClick={() => navigate('/leilao/' + auction.id)}
                  style={{ background: 'white', borderRadius: '15px', overflow: 'hidden', cursor: 'pointer', transition: 'transform 0.2s, box-shadow 0.2s', position: 'relative', boxShadow: isSameNeighborhood ? '0 0 0 3px #f97316, 0 8px 24px rgba(0,0,0,0.18)' : '0 4px 12px rgba(0,0,0,0.1)' }}
                  onMouseEnter={e => e.currentTarget.style.transform = 'scale(1.03)'}
                  onMouseLeave={e => e.currentTarget.style.transform = 'scale(1)'}
                >
                  <div style={{ position: 'relative' }}>
                    <img src={auction.images?.[0] || 'https://via.placeholder.com/300x200'} alt={auction.title} style={{ width: '100%', height: '180px', objectFit: 'cover', display: 'block' }} />
                    {timeLeft && (
                      <div translate="no" style={{ position: 'absolute', top: '8px', left: '8px', background: timeLeft.urgent ? 'rgba(220,38,38,0.93)' : 'rgba(30,58,138,0.88)', color: 'white', padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold', display: 'flex', alignItems: 'center', gap: '4px', backdropFilter: 'blur(4px)', boxShadow: '0 2px 8px rgba(0,0,0,0.3)', whiteSpace: 'nowrap' }}>
                        {timeLeft.urgent ? '🔥' : '⏳'} {timeLeft.label}
                      </div>
                    )}
                    {isAnuncio && (
                      <div style={{ position: 'absolute', top: '8px', right: '8px', background: 'rgba(249,115,22,0.93)', color: 'white', padding: '4px 10px', borderRadius: '20px', fontSize: '11px', fontWeight: 'bold', backdropFilter: 'blur(4px)', boxShadow: '0 2px 8px rgba(0,0,0,0.25)' }}>📢 ANÚNCIO</div>
                    )}
                    {auction.neighborhood && (
                      <div style={{ position: 'absolute', bottom: '0', left: '0', right: '0', background: 'linear-gradient(transparent, rgba(0,0,0,0.72))', color: 'white', padding: '18px 10px 7px', fontSize: '12px', fontWeight: '700', display: 'flex', alignItems: 'center', gap: '4px' }}>
                        📍 {auction.neighborhood}
                        {isSameNeighborhood && (
                          <span style={{ marginLeft: '4px', background: '#f97316', color: 'white', fontSize: '10px', fontWeight: 'bold', padding: '1px 7px', borderRadius: '10px' }}>Perto de você</span>
                        )}
                      </div>
                    )}
                  </div>
                  <div style={{ padding: '14px 16px' }}>
                    <h3 style={{ margin: '0 0 4px 0', fontSize: 'clamp(15px, 2vw, 19px)', lineHeight: '1.3' }}>{auction.title}</h3>
                    ...
                    <div style={{ fontSize: 'clamp(20px, 3vw, 26px)', fontWeight: 'bold', color: isAnuncio ? '#f97316' : '#667eea' }}>R$ {parseFloat(auction.current_price || 0).toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</div>
                  </div>
                </div>
              )
            })}
