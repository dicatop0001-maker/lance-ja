...
function NovoLeilao({ user, onBack, onSuccess }) {
  ...
  const handleImageChange = (e) => {
    ...
    if (files.length + images.length > 5) {
      alert('Máximo de 5 fotos por leilão')
      return
    }
      ...
      <div style={{ maxWidth: '800px', margin: '30px auto', padding: '20px' }}>
        <form onSubmit={handleSubmit} style={{ background: 'white', borderRadius: '20px', padding: '40px' }}>
          <div style={{ marginBottom: '25px' }}>
            <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold' }}>📸 Fotos do Produto (até 5)</label>
              ...
              <div style={{ display: 'flex', gap: '10px', marginTop: '15px', flexWrap: 'wrap' }}>
                {images.map((img, i) => (
                  <div key={i} style={{ position: 'relative', width: '100px', height: '100px' }}>
                    ...
                    <button type="button" onClick={() => removeImage(i)} style={{ position: 'absolute', top: '-5px', right: '-5px', background: 'red', color: 'white', border: 'none', borderRadius: '50%', width: '25px', height: '25px', cursor: 'pointer', fontSize: '14px' }}>×</button>
                  </div>
                ))}
              </div>
            )}
