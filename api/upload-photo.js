...
module.exports = async (req, res) => {
  ...
  if (!serviceKey) {
    return res.status(500).json({
      error: 'SUPABASE_SERVICE_ROLE_KEY nao configurada. Adicione no Vercel: Settings > Environment Variables',
      envKeys: Object.keys(process.env).filter(k => k.includes('SUPA') || k.includes('supabase')).join(', ') || 'nenhuma env supabase encontrada'
    })
  }
